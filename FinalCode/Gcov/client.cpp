		...
		else
		{
			if(send(sockfd, temp.c_str(), strlen(temp.c_str()),0)<0)
			{
				...
				exit(EXIT_FAILURE);
		   		...
		   		if(temp == "RT") //receiving from mainClient based on Command.
				{
					char buffData[SIZE];
					if(recvfrom(sockfd, buffData, sizeof(buffData),0,(struct sockaddr*)&client_addr, (socklen_t*)&client_addr_len)<0)
					{
